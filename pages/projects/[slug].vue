...
allSlugs.value = (slugsData.value ?? []).map(t => t.slug)
const index = allSlugs.value.indexOf(slug)

// 404 — slug topilmasa yoki nashr qilinmagan bo'lsa
if (index === -1) {
	throw createError({ statusCode: 404, statusMessage: "Sahifa topilmadi" })
}

const { data: taskData, error: taskError } = await useAsyncData(`task-${slug}`, async () => {
	...
	<main class="min-h-screen bg-white font-sans overflow-x-hidden">
		...
		<section class="relative min-h-[70vh] flex items-end overflow-hidden bg-[#05080f]">
			<div class="absolute inset-0 z-0">
				...
				<div class="hero-grid" />
				<div
					class="absolute inset-0 bg-gradient-to-t from-[#05080f] via-[#05080f]/40 to-transparent"
				/>
			</div>

			<div class="relative z-10 w-full max-w-6xl mx-auto px-6 pb-20 pt-40">
				<div
					class="inline-flex items-center gap-2 bg-blue-600/15 border border-blue-500/30 rounded-full px-4 py-1.5 mb-6 from-top"
				>
		...
		<section class="py-24 px-6 bg-white">
			<div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-16">
				...
				<div class="lg:col-span-2 space-y-16">
					...
					>
						<div class="flex items-start gap-4 mb-5">
							<div
								class="w-1 h-8 bg-blue-600 rounded-full flex-shrink-0 mt-1"
							/>
							<h2
								class="text-2xl md:text-3xl font-black text-slate-900 tracking-tight leading-tight"
							>
								{{ section.heading }}
							</h2>
						</div>
						...
						<p
							v-if="section.text"
							class="text-slate-500 leading-relaxed text-base mb-5 ml-5"
						>
							{{ section.text }}
						</p>
					...
					<div
						...
						class="text-slate-300 text-sm italic"
					>
				...
				<aside class="from-right space-y-6">
					...
					>
						<p
							class="text-blue-400 text-[10px] font-black uppercase tracking-[0.4em] mb-4"
						>
						...
						<ul class="space-y-4">
							...
							>
								<span
									class="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-[10px] font-black flex-shrink-0 mt-0.5"
								>
									{{ i + 1 }}
								</span>
								{{ result }}
					...
					<div class="border border-slate-100 rounded-3xl p-6 text-center">
						<div
							class="text-6xl font-black text-slate-100 tracking-tight leading-none mb-1"
						>
		...
		<section class="py-16 bg-slate-50 border-t border-slate-100 px-6">
			<div class="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between gap-4">
				...
				>
					<div
						class="w-10 h-10 rounded-full border border-slate-200 group-hover:border-blue-500 group-hover:bg-blue-600 flex items-center justify-center transition-all duration-300"
					>
						<svg
